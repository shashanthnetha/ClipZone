...
def execute_production_pipeline(
    ...
    print("\n🚀 [STAGE 2B] AI Director Engine...")
    start_dir = time.time()
    from clippilot.brain.director_engine import make_creative_blueprint
    
    # Extract decision, or construct a fallback StrategyDecision
    ...
    if "decision" in locals():
        ...
        strategy_decision = StrategyDecision(
            ...
            confidence=0.20,
            reasoning="Fallback strategy decision from rotation.",
            variation=variation
        )
        
    try:
        stats_summary = learning_engine.generate_summary()
        history_records = learning_engine.history
    except Exception:
        stats_summary = None
        history_records = None
        
    blueprint = make_creative_blueprint(
        topic=topic,
        decision=strategy_decision,
        learning_summary=stats_summary,
        history=history_records
    )
    timings["ai_director"] = round(time.time() - start_dir, 4)
    print(f"✔️ Creative Blueprint generated by Director:")
    import json
